...
async def global_ban(event):
    if event.fwd_from:
        return
    await event.edit("`Gban dulu njinggg!...`")
    start = datetime.now()
    ...
    for i in range(len(groups_admin)):
        try:
            await cat.client(EditBannedRequest(groups_admin[i], user.id, BANNED_RIGHTS))
...
async def unglobal_ban(event):
    ...
    for i in range(len(groups_admin)):
        try:
            await cat.client(EditBannedRequest(groups_admin[i], user.id, UNBAN_RIGHTS))
            await asyncio.sleep(0.5)
            count += 1
        except BadRequestError:
            await event.client.send_message(
